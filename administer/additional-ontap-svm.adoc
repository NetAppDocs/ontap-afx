...
CFSR is enabled by default on AFX systems running ONTAP 9.19.1 or later, but you need to explicitly enable it on the individual volumes. Storage administrators can configure the feature using the CLI or equivalent REST API call. With the CLI, you can create a new volume with CFSR enabled. You can also update an existing volume with the following command:
