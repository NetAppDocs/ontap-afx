...
SnapMirror active sync allows business services to continue functioning in the event of a complete site failure. This technology enables applications to fail over seamlessly to a secondary copy without manual intervention or custom scripting. Learn more about https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync for NAS^].
...
Beginning with ONTAP 9.19.1, ONTAP adds on-premises support for NFS over TLS with TLS 1.3 to encrypt NFS traffic in transit on NFS-enabled SVMs. The release also supports optional per-LIF mutual TLS host authentication and export-policy controls to deny non-TLS TCP access for matching clients. NFS over TLS works with existing ONTAP NFS infrastructure, so applications continue to use NFS normally while TLS encrypts NFS traffic in transit. NFS over TLS is not available for Cloud platforms in ONTAP 9.19.1. Refer to https://docs.netapp.com/us-en/ontap/nfs-admin/enable-nfs-over-tls-task.html[Enable NFS over TLS^] for more information.
