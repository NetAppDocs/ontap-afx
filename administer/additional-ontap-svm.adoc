...
CFSR can operate using FlexGroup volumes with GDD enabled or disabled. However, because video frame files are typically 50-250MB (maximum 800MB for higher resolution) and GDD's minimum segment size is 1GB, GDD does not stripe these files effectively across FlexGroup constituents. This means GDD has minimal direct impact on CFSR's file prefetching behavior.
