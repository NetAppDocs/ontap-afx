...
[.lead]
The ports used by ONTAP AFX provide the foundation for network connectivity and communication. There are several configuration options available to customize this layer two aspect of your AFX network.
...
. In System Manager, select *Network* and then *Ethernet ports*.
. Select image:icon_lag.png[a blue plus sign followed by the word lag].
