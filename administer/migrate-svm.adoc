...
[.lead]
You can migrate a data SVM from one ONTAP cluster to another. SVM migration with AFX operates the same as with Unified ONTAP, although there are several interoperability considerations. Refer to the provided Unified ONTAP documentation for details about performing an SVM migration.
