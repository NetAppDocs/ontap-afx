...
[.lead]
After your AFX hardware has been installed, you can complete the ONTAP cluster setup. This process involves related configuration phases that you should perform sequentially. As part of this, you can optionally add DX50 data compute nodes (DCNs) running the AI Data Engine (ADIE), enabling the AFX storage cluster and DCN cluster to operate as a single system that's managed through the same ONTAP control plane.
...
* Ensure the https://docs.netapp.com/us-en/ai-data-engine/install-setup/install-setup-workflow.html[DX50 data compute nodes have been installed^] and attached to the AFX storage network.
...
* If you have included AIDE as part of the AFX cluster setup, you'll need to perform additional configuration steps including licensing and authentication. See https://docs.netapp.com/us-en/ai-data-engine/install-setup/cluster-setup-afx.html[Set up AIDE with a new AFX cluster^] for more information.
