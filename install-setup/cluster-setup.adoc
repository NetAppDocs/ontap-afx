...
[.lead]
After your AFX hardware has been installed, you can complete the ONTAP cluster setup. This process involves related configuration phases that you should perform sequentially. As part of this, you can optionally add DX50 data compute nodes (DCNs) running the AI Data Engine (ADIE), so that the AFX storage cluster and DCN cluster operate as a single system managed through the same ONTAP control place.
...
* Ensure the DX50 data compute nodes have been properly


 installed and attached to the AFX storage network.



* Obtain the addresses and related configuration for the AIDE DCN nodes.
...
. Optionally provide the IP addresses for the AIDE DCN nodes.
...
* https://docs.netapp.com/us-en/ai-data-engine/install-setup/install-setup-workflow.html[Install your AIDE system^]
