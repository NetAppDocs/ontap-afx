...
Note that SnapLock is supported in the first release of the ONTAP AFX. In addition, FlexCache volumes can be attached to or from Unified ONTAP to or ONTAP AFX.
...
Data migration from non-ONTAP systems involves file-level copy operations. Fast copy utilities such as XCP or CloudSync can be used as well as standard copy utilities like RoboCopy (for SMB), rsync (for NFS), or third-party tools such as DataDobi.

== Limitations

Existing Unified ONTAP systems can replicate to ONTAP AFX if the data volume: 

* Does not contain LUNs or NVMe namespaces
* If replicating to a Unified ONTAP system the minimum supported ONTAP version for the Unified ONTAP system is 9.16.1. This is the first ONTAP release supporting Advanced Capacity Balancing.
