...
There are several limits, including minimums and maximums, enforced by ONTAP AFX when configuring and using your cluster. These limits fall into several different categories, including:

* Nodes per cluster
* Storage capacity

NetApp expects to expand the AFX capabilities and so these limitations will change, possibly frequentlky and within ONTAP releases. You should review the information available at the Interoperability Matrix Tool (IMT) to determine the capabilities of your cluster.
