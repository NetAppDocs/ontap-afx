...
No. Unified ONTAP is the predominant architecture and will remain so for the foreseeable future. There is no plan to require or force migration to ONTAP AFX. You can choose one or more of the ONTAP personalities (Unified ONTAP, ONTAP AFX, ASA r2) based on your needs.
...
.Can I issue ONTAPI (ZAPI) API calls to an ONTAP AFX cluster?

No. Only the REST API is supported with ONTAP API. Any automation code using ZAPI needs to be converted to REST API.
