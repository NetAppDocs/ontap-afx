...
Types of SVMs::
An AFX system cluster hosts several different types of SVMs. A *data SVM* is used to serve data to the AFX clients and is the one type an AFX administrator can directly configure. There is one data SVM created when you set up and initially deploy an AFX cluster, but you can create additional data SVMs if needed. When referring to an SVM in this documentation, a data SVM is implied unless otherwise noted.
