...
While you can use System Manager for most ONTAP AFX administration, there are some taks you can only perform using the ONTAP CLI.

.About this task

Yuo can access the ONTAP AFX CLI through the secure shell (SSH). The CLI has two primary privilege levels which determine the commands and command parameters available to you. The `admin` level is the default and has the least privilege. You can elevate the privilege of your session to `advanced` if needed.
