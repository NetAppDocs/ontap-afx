...
Storage capacity::
This is the total capacity in the collection of SSD disks in the cluster storage pool. The maximum number of nodes varies based on the release.

NetApp expects to expand the AFX capabilities and so these limitations will change, possibly frequently and even within an ONTAP release. You should review the information available at the Interoperability Matrix Tool (IMT) to determine the capabilities of your cluster.
