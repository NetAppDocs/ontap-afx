...
[NOTE]
Beginning with ONTAP 9.19.1, SVM administrators can monitor and manage their own SVMs using a dedicated System Manager dashboard. Learn more about the https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html[ONTAP System Manager SVM administrator dashboard^]
...
There are several limitations, including minimums and maximums, enforced by AFX when configuring and using your cluster. These limits fall into several categories.

[CAUTION]
Before increasing an operational parameter of your AFX storage system, you should assess how it might impact your applications as well as the cluster as a whole. For example, while expanding the number of controller nodes increases overall capacity, it could also increase traffic in the cluster network. It's important to understand the implication of these configuration changes and how they might impact your environment.

Controller nodes per cluster::
Each AFX cluster must have at least four nodes. The maximum number of nodes varies based on the ONTAP release; for example the 9.19.1 RC release allows up to 32 nodes per AFX cluster.
...
You should review the details available at the NetApp Hardware Universe and Interoperability Matrix Tool to determine the capabilities of your AFX cluster. Also review link:../release-notes/changes-to-limits-defaults.html[Changes to ONTAP limits and defaults] in AFX release notes section.
