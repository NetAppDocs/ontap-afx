...
Unlike other NetApp ONTAP storage systems, the compute and storage elements of an AFX cluster are decoupled. Disk ownership is no longer associated with specific nodes which provides several benefits. For example, the compute and storage components of a cluster can be increased independently.
...
The physical aggregates are no longer available to the AFX storage administrator. Instead, AFX automatically manages the virtual capacity allocations for nodes, as well as RAID group configuration, when new storage shelves are added. This design simplifies administration and provides an opportunity for nonspecialists to manage their data. 
...
High performance::
NetApp AFX is built for high performance NAS and object workloads, and provides high and sustained bandwidth with ultra-low latency. AFX uses the latest modern hardware as well as storage shelves capable of handling a high ratio of nodes to disks made possible by its unique architecture. Scaling storage nodes beyond the typical 1:1 (node:shelf) ratio maximizes the total possible performance profile of the disks to their edge limits. This design provides efficiency and density to your most critical applications.
