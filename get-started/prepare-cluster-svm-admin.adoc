...
// Need to confirm usage of the terms "storage virtual machine" and "SVM". Both are acceptable with Acrolinx.

A storage virtual machine (SVM) is an isolated server or tenant environment within an ONTAP cluster. SVMs are configured to serve data to the connected clients. One data SVM is created by default with a new ONTAP AFX cluster and you can create additional SVMs if needed. You should be familiar with the capabilities and characteristics of ONTAP AFX SVMs.
...
It's important to note there are two levels of authentication and authorization with ONTAP AFX: cluster level and SVM level. Every SVM has its own set of users and roles which are distinct and separate from the cluster accounts. In most situations, using the cluster level accounts is adequate. But depending on your environment, you might need to configure the more restrictive SVM accounts and roles as well. See link:../administer/advanced-svm.html[ONTAP AFX advanced SVM administration] for more information.

.SVM-scoped resources

ONTAP AFX resources and configurable entities are associated either with the entire cluster or with a specific SVM. There are many resources with an SVM scope, including volumes and buckets as well as the SVM user accounts and RBAC roles.
...
|REST API
|ONTAP AFX includes a REST API you can use to automate the administration of your AFX cluster. As with AFX in general, the API is based on the unified ONTAP REST API with modifications to support the unique AFX features and restrictions.
|===
...
Before performing any of the ONTAP AFX administration tasks, you should check the health of the cluster.
...
. Display the system events and audit log
+
link:../administer/view-events-log.html[View ONTAP AFX events and audit log]

. Display and note any *Insight* recommendations.
+
link:../administer/view-insights.html[Optimize ONTAP AFX cluster performance and security with insights]

== Quick start for creating and using data SVMs

After installing and setting up the ONTAP AFX cluster, you can begin performing the administration tasks typical of most AFX deployments. Here are the steps needed to begin sharing data with clients using a storage VM.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-1.png[One] Display the available data SVMs

[role="quick-margin-para"]
link:../administer/display-svm.html[Display] the list of data SVMs and determine if there's one you can use.
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-4.png[Four] Create a volume

[role="quick-margin-para"]
link:../manage-data/create-manage-volume.html[Create] a volume attached to your ONTAP AFX SVM.
